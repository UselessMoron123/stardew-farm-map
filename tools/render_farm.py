        ...
        for x in range(W):
            ...
            if b is not None:
                ...
                if isinstance(b, StaticTile):
                    if b.sheet == MAIN and b.index in WATER: c = 'water'
                    elif b.sheet == 'v16_Outdoors2' and b.index in S2_WATER: c = 'water'
                    elif b.sheet == MAIN and b.index == 587: c = 'grass'
                    elif b.sheet == MAIN and b.index == 176: c = 'cliff'
                    else: c = 'floor'
            if bld is not None:
                if isinstance(bld, StaticTile) and bld.sheet == MAIN and bld.index == 16: c = 'block'
                elif isinstance(bld, StaticTile) and bld.sheet == MAIN and bld.index == 1957: c = 'sign'
                else: c = 'solid'
            if p is not None and isinstance(p, StaticTile) and p.sheet == 'Paths':
                if p.index in (9,10,11,12,23,31,32): c = 'tree'
                elif p.index in (29,30): c = 'cabin'
                elif p.index in (22,36): c = 'ginit'
                elif p.index in (13,14,15,16,17,18,19,20,21): c = 'debris'
                elif p.index in (24,25,26): c = 'bush'
            row.append(c)
        grid.append(row)
    return grid
